...
import (
	...
	"github.com/moontrade/kirana/logger/slog/internal"
)
...
type HandlerRaw interface {
	//HandleRaw(ts int64, pc uintptr, level Level, msg string, attr ...Attr) error
	HandleRaw(record Record, attr ...Attr) error
}
...
// LogAttrs is a more efficient version of [Logger.Log] that accepts only Attrs.
func (l *Logger) LogAttrsRaw(ctx context.Context, level Level, msg string, attrs ...Attr) {
	l.logAttrsRaw(level, msg, attrs...)
}
...
func (l *Logger) logAttrs(ctx context.Context, level Level, msg string, attrs ...Attr) {
	...
	r := NewRecord(time.Now(), level, msg, pc)
	//r := NewRecord(time.Time{}, level, msg, pc)
	r.AddAttrs(attrs...)
	if ctx == nil {
		ctx = context.Background()
	}
...
// logAttrs is like [Logger.log], but for methods that take ...Attr.
func (l *Logger) logAttrsRaw(level Level, msg string, attrs ...Attr) {
	var pc uintptr
	_ = pc
	if internal.IgnorePC {
		var pcs [1]uintptr
	...
	//_ = l.HandlerRaw.HandleRaw(0, pc, level, msg)
	//_ = l.HandlerRaw.HandleRaw(r)
}
