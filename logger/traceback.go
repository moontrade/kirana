...
func getCallerPCSlow0() *FuncInfo {
	gp := getg()
	var f stkframe
	var pcbuf [3]uintptr
	systemstack(func() {
		//f = traceback(^uintptr(0), ^uintptr(0), 0, gp, 2, &pcbuf[0], len(pcbuf))
		f = traceback(^uintptr(0), ^uintptr(0), 0, gp, 0, nil, len(pcbuf))
	...
	pc := *(*uintptr)(unsafe.Pointer(f.prevSP))
	_ = pc
	pc = pcbuf[0]
	pc = f.pc
...
func getCallerFunc0() *FuncInfo {
	return getCallerPCSlow()
}

func getCallerPCSlow() *FuncInfo {
	pc, file, line, ok := runtime.Caller(1)
	_ = file
	_ = line
	_ = ok
	return funcInfoMap.GetForPC(pc)
}
func getCallerPCSlowSlow() {
	pc, file, line, ok := runtime.Caller(1)
	if !ok {
		return
	}
	println(file + ":" + strconv.FormatInt(int64(line), 10))
	...
	//		continue
	//	}
	//	file, line := f.Func.FileLine(f.PC)
	//	println("___", file+":"+strconv.FormatInt(int64(line), 10))
	//	if !more {
	//		return
	//	}
	//}
